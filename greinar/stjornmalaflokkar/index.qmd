...
if (!file.exists("data/data.csv")) {
  ...
  url <- "https://docs.google.com/spreadsheets/d/1HgTvLgpZV2vizu8fHU9mXFMNn1Uim2sKLB5gP2I_xuo/edit?gid=0#gid=0"
  ...
  d |> 
    write_csv(here::here("data/data.csv"))
  
} else {
  d <- read_csv(here::here("data/data.csv"))
}
```
...
dreifing_plot <- function(data, title, breaks = 2007:2024, labels = 2007:2024) {
  plot_dat <- data |> 
...
magn_plot <- function(data, title, breaks = 2007:2024, labels = 2007:2024) {
  plot_dat <- data |> 
    select(ar, flokkur, value, litur) |> 
    group_by(ar) |> 
    mutate(p = value / sum(value)) |> 
...
d |> 
  ...
  dreifing_plot(title = "Dreifing framlaga einstaklinga",
                breaks =  seq(2007, 2022, by = 2),
                labels =  seq(2007, 2022, by = 2))

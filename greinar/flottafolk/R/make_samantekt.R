make_samantekt <- function(
  ...
  p11 <- d |>
    ...
    labs(
      ...
      subtitle = glue(
        "Fjöldi í {month(end_date[1], label = T, abbr = F)} {year(end_date)}"
      )
    )
  ...
  p1 <- p11 + p12 + plot_layout(nrow = 1, widths = c(0.8, 1))
  ...
  p21 <- d |>
    ...
    labs(
      ...
      subtitle = glue(
        "Fjöldi í {month(end_date[2], label = T, abbr = F)} {year(end_date[2])}"
      )
    )
  ...
  p2 <- p21 + p22 + plot_layout(nrow = 1, widths = c(0.8, 1))
  ...
  p31 <- d |>
    ...
    labs(
      ...
      subtitle = glue(
        "Fjöldi í {month(end_date[3], label = T, abbr = F)} {year(end_date)}"
      )
    )
  ...
  p3 <- p31 + p32 + plot_layout(nrow = 1, widths = c(0.8, 1))

make_ggiraph2 <- function(
    ...
    mutate(
      tooltip = glue(
        str_c(
          ...
          "Fjöldi (2024): {number(value[year(time)==2024], accuracy = 1, big.mark = '.', decimal.mark = ',')}\n",
          "Á 100.000 íbúa (2024): {number(per_pop[year(time)==2024], accuracy = 1, big.mark = '.', decimal.mark = ',')}"
        )
      ),
      tooltip = if_else(
        time == max(time),
  ...
  p2 <- plot_dat |>
    ...
    labs(
      ...
      subtitle = "1: Flestar veitingar | 31: Fæstar veitingar"
    )
  ...
  plot_dat <- data_hist |>
    ...
    mutate(
      tooltip = glue(
        str_c(
          "Uppsafnaður fjöldi (2024): {number(value[year(time) == 2024], accuracy = 1, big.mark = '.', decimal.mark = ',')}\n",
          "Á 100.000 íbúa (2024): {number(per_pop[year(time) == 2024], accuracy = 1, big.mark = '.', decimal.mark = ',')}"
        )
      ),
      tooltip = if_else(
  ...
  p <- p1 +
    ...
    plot_annotation(
      title = glue(
        "Hælisveitingar í Evrópulöndum (2008 til {year(end_date)} {title_addon})"
      ),
      subtitle = str_c(
